...
const getElectrons = (count, offset = 0, type=0) =>
  Array.from({ length: count }, (v, k) => k).map(k => ({
    id: `particula-${k + offset}-${new Date().getTime()}`,
    content: `-`
  }));

  const getProtons = (count, offset = 0, type=0) =>
  Array.from({ length: count }, (v, k) => k).map(k => ({
    id: `particula-${k + offset}-${new Date().getTime()}`,
    content: `+`
  }));

  const getNeutrons = (count, offset = 0, type=0) =>
  Array.from({ length: count }, (v, k) => k).map(k => ({
    id: `particula-${k + offset}-${new Date().getTime()}`,
    content: `.`
  }));
...
const getParticleStyle = (isDragging, draggableStyle, type) => ({
  ...
  backgroundColor: (type==="+") ? "red" : (type==="-") ? "blue" : "black",

  

  // estilo cuando desplaza
...
export function Cubeta(props) {
  ...
  function onDragEnd(result) {
    ...
    // regresar si se puso fuera de la lista
    if (!destination) {
      return;
    }
    ...
    if(source.droppableId==='0' && destination.droppableId==='3'){
      electrones++;
    }
    ...
    if(source.droppableId==='3' && destination.droppableId==='0'){
      electrones--;
    }
    if(source.droppableId==='3' && destination.droppableId==='1'){
      protones--;
    }
    ...
    //console.log(source);
    let numeroelemento= "n"+ protones.toString();
    
    elemento_actual=infoelementos.elements[numeroelemento];

    props.handler(elemento_actual,electrones,protones,neutrones);

    //console.log(electrones);
    //console.log(protones);
    //console.log(neutrones);
